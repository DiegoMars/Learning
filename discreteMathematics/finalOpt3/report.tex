...
\usepackage{amsmath} % For the question* environment
...
\renewcommand{\footrulewidth}{0pt}  % Removes the horizontal line above the 
                                    % footer.
%To make sure we have header 0.5in away from top edge
...
\section{\textbf{Shamir's Secret Sharing and its purpose}}
...
there are enough shares, or meet the threshold of shares, which is denoted by a
number $K$.
...
also recreate the secret. Let's say a polynomial $y = ax + b$. The $b$ in this
...
would be the secret. Using modulus can also be used in SSS and will be
covered later.

\section{\textbf{Mathematical background}}
In order to implement SSS, there are some concepts to go over first. The first
which is the \textbf{Lagrange Polynomial Interpolation}, which is used to find
the polynomial of the lowest degree that goes through all of the shares. A
Lagrange polynomial of some share's $x$ coordinate $x_i$, is a polynomial that
its $y$ value at that $x_i$ is $1$, and is $0$ for every other share's, at leas
the shares inputted, $x$ coordinate.

For example, let's say there are some $x$ coordinates $x=1,2,3,4$, the Lagrange
...
Here are the equations for Lagrange Polynomial Interpolation.
\begin{align*}
...
Walking through the full example of $x=1,2,3,4$, the following Lagrange
...
\begin{align*}
 f(x) = x + 4
\end{align*}
Plotting the line $f(x)=x+4$, it does indeed go through the points $(1,5),(2,6),
(3,7),(4,8)$.
\begin{center}
\begin{tikzpicture}
\begin{axis}[
...
where the remainder of the division. For example, the remainder of 8 divided by
6 is 2. Equivalently, $8 \bmod 6$ would be $2$. When applying this to an
...
\Function{generateY}{polynomial, $x$, modulus}
    ...
    \For{i \textbf{to} (len(polynomial)-2)}
        \State $y := y + x^{i+1}$
    \EndFor
...
\State degree $:= K-1$ 
\State
% Get the biggest number out of S,K,N and calculate the modulus accordingly
...
Lets move on to the fun part, \textbf{the decryption}. Note that the modulus
must be known to recreate the secret, as to my knowledge, there is no way to
calculate the modulus with just the shares. When it comes to mod, remember that
we cannot do regular division when under $\bmod$ and that we would need to find
the $\bmod$ inverse of the divisor then multiply it by the dividend. I am not
sure if it is correct, but it works for me, but after getting the Lagragne
polynomial's values at $x=0$, multiply by there respective $y$ values, then
added them all up, we have to do the $\bmod$ of that final value.
\textbf{Now lets go over the steps for decryption:}
\begin{enumerate}
    \item Input some number of shares and the modulus
    \item Find the value of the Lagrange polynomial for every point at $x=0$
        \begin{itemize}
            \item To find the Lagrange polynomial, the $\bmod$ inverse must be
                    found for the divisor
        \end{itemize}
    \item Multiply each value by their respective $y$ value
    \item Add up all of the values, then do $\bmod$ modulus
\end{enumerate}

\begin{algorithm}[H]
\caption{Decryption}
\begin{algorithmic}[1]
\Statex \textbf{Input:} share, or a list of points, and modulus
\Statex \textbf{Output:} secret, or the decrypted secret from the points
% Modular Inverse
\Function{modInverse}{$\text{number}, \text{modulus}$}
    \State inverse $:= 1$
    \While{($\text{number} \cdot \text{inverse}) \bmod \text{modulus} \neq 1$}
        \State inverse $:= \text{inverse} + 1$
    \EndWhile
    \State \Return inverse
\EndFunction
\State
% Lagrange coefficient at x = 0
\Function{lagrange}{$\text{position}, \text{shares}, \text{modulus}$}
    \State dividend $:= 1$
    \State divisor $:= 1$
    \For{$i := 0$ \textbf{to} len(shares)$-1$}
        \If{$i = \text{position}$}
            \State \textbf{Continue}
        \EndIf
        \State $x_j := \text{shares}[i][0]$
        \State $x_i := \text{shares}[\text{position}][0]$
        \State dividend $:= \text{dividend} \cdot (-x_j)$
        \State divisor $:= \text{divisor} \cdot (x_i + (-x_j))$
    \EndFor
    \State inverse $:=$ \Call{modInverse}{divisor, modulus}
    \State lagrangeValue $:=$ dividend $\cdot$ inverse
    \State \Return lagrangeValue
\EndFunction
\State
% Decryption from shares
\State secret $:= 0$
\State lagragneValues $:=$ []
\For{$i := 0$ \textbf{to} len(shares)$-1$}
    \State lVal $:=$ \Call{lagrange}{$i$, shares, modulus}
    \State lagragneValues.append(lVal)
\EndFor
\For{$i := 0$ \textbf{to} len(shares)$-1$}
    \State secret $:= \text{secret} + \text{shares}[i][1] \cdot 
            \text{lagragneValues}[i]$
\EndFor
\State secret $:=$ secret $\bmod$ modulus
\State \Return secret
\end{algorithmic}
\end{algorithm}

The first function \Call{modInverse}{ } brute forces looking for the $\bmod$
divisor, where it will loop through numbers until it finds it. The proper way of
finding the $\bmod$ inverse is using the \textbf{Extended Euclidean Algorithm},
but chose to just brute force it instead.

Note that I first made the coding implementation of both the encryption and
decryption algorithms of SSS, and translated it to Pseudocode. The coding
implementation can be found here: 
\url{https://www.jdoodle.com/ga/eAkyDME7ZOQgF91souI%2B7g%3D%3D}

\section{\textbf{Security Analysis}}

In SSS, it would be impossible to reconstruct the secret without having enough
shares to meet the threshold. The modular arithmetic makes it harder to gain
some information about the secret.

The assumptions here are that the number of shares $N$, the threshold $K-1$, and
the modulus is known. No matter what, you cannot reconstruct the secret without
having enough shares to meet the threshold. Because of this, the SSS encryption
algorithm does not rely on how hard it is to compute the secret, but
on a mathematical property.

The point of failure when it comes to SSS, would not be the algorithm itself but
on how secure the storage of the shares is, and things like that.

\section{\textbf{Real-world relevance}}

In the real world, the idea of splitting up a secret among multiple people, and
and only reconstructing it given enough people to agree, is a concept that is
used in digital wallets, vaults, or any other systems where you would want
multiple people with permission to access something. One can say that something
like nuclear launch codes can also be an example.
