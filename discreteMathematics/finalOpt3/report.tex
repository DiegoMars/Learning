...
Walking through the full example of $x=1,2,3,4$, the following Lagrange polynomials
are found:
\begin{align*}
...
where the remainder of the division. For example, the remainder of 8 divided by 6 is
...
Lets move on to the fun part, \textbf{the decryption}. Note that the modulus must
...
we cannot do regular division when under $\bmod$ and that we would need to find the
$\bmod$ inverse of the divisor then multiply it by the dividend. I am not sure if it
...
up, we have to do the $\bmod$ of that final value. \textbf{Now lets go over the 
